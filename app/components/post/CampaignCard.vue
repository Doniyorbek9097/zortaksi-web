<template>
  ...
  >
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2">
          ...
          <span
            ...
            class="shrink-0 text-[11px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full bg-emerald-500 text-white"
          >
            Faol
          </span>
        </div>
        <p class="text-[13px] font-medium text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 leading-snug">
          {{ campaign.text }}
        </p>
        <p class="text-[12px] font-semibold text-slate-400 mt-1.5">
        ...
        <p v-if="campaign.lastError" class="text-[12px] font-bold text-rose-500 mt-1">
          {{ campaign.lastError }}
        </p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-1.5 flex-wrap">
      <button
        type="button"
        class="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-[13px] font-black transition-all active:scale-95"
        :class="campaign.active
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-[13px] font-black text-sky-600 dark:text-sky-400 border border-sky-200 dark:border-sky-900/50 bg-sky-500/5 active:scale-95"
        :disabled="busy"
        @click="$emit('edit')"
      >
        <font-awesome-icon icon="fa-solid fa-pen-to-square" class="text-[11px]" />
        Tahrirlash
      </button>

      <button
        type="button"
        class="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-[13px] font-black text-rose-600 dark:text-rose-400 bg-rose-500/10 border border-rose-200 dark:border-rose-900/50 active:scale-95 disabled:opacity-50"
        :disabled="busy"
        @click="$emit('delete')"
      >
